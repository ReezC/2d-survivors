...
@onready var dead_anim_player: AnimationPlayer = $AnimationPlayer
...
# 眨眼分两种模式：
#   1. 单眨：只播放一次 blink 动画
#   2. 连眨：播放第一次 → 短暂恢复 default → 播放第二次
# 每次触发时按 FACE_BLINK_DOUBLE_CHANCE 概率决定连眨。

var _face_is_blinking: bool = false             # 当前是否正在播放 blink 动画
var _face_blink_remaining: int = 0              # 本次剩余 blink 次数（1=单眨, 2=连眨）
var _face_blink_gap_ms: float = 0.0             # 连眨时两次 blink 之间的间隔计时器
var _face_blink_interval_ms: float = 3000.0     # 下次眨眼触发的随机间隔（会在 min~max 间重设）
var _face_blink_timer_ms: float = 0.0           # 距上次眨眼结束的累计计时
var _face_blink_frame: int = 0                  # 当前 blink 动画播放到的帧号
var _face_blink_frame_timer_ms: float = 0.0     # blink 动画每帧停留的累计计时
const FACE_BLINK_FRAME_DELAY_MS := 60.0        # blink 动画每帧持续时间（毫秒）
const FACE_BLINK_SEQUENCE_GAP_MS := 150.0      # 连眨两次时，第一次与第二次之间的间隔（毫秒）
const FACE_BLINK_INTERVAL_MIN_MS := 2000.0     # 两次眨眼触发的最小随机间隔（毫秒）
const FACE_BLINK_INTERVAL_MAX_MS := 6000.0     # 两次眨眼触发的最大随机间隔（毫秒）
const FACE_BLINK_DOUBLE_CHANCE := 0.3          # 连眨两次的概率（0.0~1.0）
...
## 覆写死亡逻辑：死亡时按基础槽位（Bd/Hd/Hr/Fc）重建纸娃娃
func die() -> void:
	if paper_doll:
		paper_doll.rebuild_for_death()
	if dead_anim_player and dead_anim_player.has_animation("dead"):
		dead_anim_player.active = true
		dead_anim_player.play("dead")
	super.die()
...
func player_move(delta: float) -> void:
	...
	if 当前状态 != 角色状态.施法 or 施法允许改朝向:
		...
		# 同步 facingDirection 的水平分量，供死亡特效等翻转判断
		if h_sign != 0:
			facingDirection.x = h_sign
