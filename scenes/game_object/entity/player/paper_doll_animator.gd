	...
	if _blink_active:
		...
		if _blink_frame_timer_ms >= BLINK_FRAME_DELAY_MS:
			...
			if _blink_frame >= blink_max:
				...
				_restore_face_animation(face_node)
			else:
				# 下一帧
...
func _restore_face_animation(face_node: VisualItemPart) -> void:
	"""眨眼结束后恢复 face 到主动画"""
	if _current_anim.is_empty():
		return
	# 查找 face 部件在当前动画中的帧
	for config_data in _builder.get_all_configs().values():
		...
		var frame: Dictionary = frames[frame_idx]
		for sprite_cfg in frame.get("spritecfg", []):
			if sprite_cfg.get("$type", "").ends_with(".Sprite") and sprite_cfg.get("name") == "face":
				var anim_name: String = anim_data.get("name", "")
