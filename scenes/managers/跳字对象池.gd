...
# 颜色配置
const 玩家受到的普通伤害颜色 = Color("#FF0000")  # 红色
const 玩家受到的暴击伤害颜色 = Color("#FF0000")  # 橙红色
const 玩家格挡后的伤害颜色 = Color("#708090")  # 道奇蓝
const 玩家闪避颜色 = Color("#00FFFF")  # 青色

const 怪物受到的普通伤害颜色 = Color("#D3D3D3")  # 浅灰色
const 怪物受到的暴击伤害颜色 = Color("#FFD700")  # 明黄色
const 怪物格挡后的伤害颜色 = Color("#FFFFFF")  # 白色
const 怪物闪避颜色 = Color("#FFFACD")  # 青色

# 大小配置
const 玩家受到的普通伤害大小 = 1.0
const 玩家受到的暴击伤害大小 = 1.2
const 玩家格挡后的伤害大小 = 1.0
const 玩家闪避大小 = 1.0

const 怪物受到的普通伤害大小 = 1.0
const 怪物受到的暴击伤害大小 = 1.0
const 怪物格挡后的伤害大小 = 1.0
const 怪物闪避大小 = 1.0
...
const 跳字基础Z层级: int = 1000

func 显示跳字(位置: Vector2, 内容: String, 跳字类型:跳字类型枚举,Z层级: int = 0) -> Node:
	var 跳字 = 获取跳字()
	...
	match 跳字类型:
		...
		跳字类型枚举.怪物闪避:
			...
			_大小 = 怪物闪避大小
		_:
			push_error("[color=red]跳字对象池：未知的跳字类型枚举 %s[/color]" % str(跳字类型))
			
	跳字.设置跳字内容(内容, _颜色)
	跳字.name = "战斗跳字_%d" % 跳字ID
	跳字.z_index = 跳字基础Z层级 + Z层级
	跳字.大小倍率 = _大小
...
func 显示治疗数字(目标位置: Vector2, 治疗值: int) -> Node:
	return 显示跳字(目标位置, "+%d" % 治疗值, 跳字类型枚举.玩家受到的普通伤害)
