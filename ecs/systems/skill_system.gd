...
## 按实体存储 AI 类型覆盖（eid -> AI类型枚举）
## 仅在 SkillManager 的 skill_ai == 无 时写入，用于禁用特定实体的 AI
var _entity_ai_override: Dictionary = {}  # eid -> int
...
func _create_skill_component(skill_res: 技能数据, caster: Node, eid: int) -> SkillComponentData:
	...
	scd.动画名称 = skill_res.技能逻辑.get("buff", {}).get("animation", {}).get("animationName", "")
...
## 设置实体的技能 AI 覆盖（由 SkillManager 调用）
func set_entity_skill_ai(eid: int, ai_type: int) -> void:
	_entity_ai_override[eid] = ai_type
...
func _update_enemy_ai(delta: float) -> void:
	...
	for eid in entities:
		...
		if not "enemy" in caster.get_groups():
			continue
		# 如果该实体的 skill_ai 被显式设为"无"，跳过自动释放
		if _entity_ai_override.get(eid, -1) == AI类型枚举.无:
			continue
		# 检查死亡
...
func _cast_skill_internal(entity_id: int, scd: SkillComponentData) -> void:
	...
	# 如果技能有施法动画且施法者正在播放动画，阻止重复释放
	if scd.动画名称 != "" and caster and caster.has_method("get") and "当前状态" in caster:
		if caster.当前状态 == caster.角色状态.施法:
			GMLogger.log_skill("[%s] 技能释放被阻止: %s (id=%d) — 正在施法中" % [_owner_name(entity_id), scd.技能名称, scd.id])
			return
